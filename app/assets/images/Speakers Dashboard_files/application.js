...
(function() {
  ...
  Dashing.Countdown = (function(_super) {
    ...
    Countdown.prototype.startCountdown = function() {
      var current_timestamp, d, dayname, end_timestamp, h, i, m, s, seconds_until_end, w, _i, _results;
      ...
      if (seconds_until_end < 3600) {
        return $(this.node).parent().remove();
        ...
        if (d > 0) {
          ...
          this.set('timeleft', d + " " + dayname + " " + this.formatTime(h) + ":" + this.formatTime(m) + ":" + this.formatTime(s));
          this.set('daysleft', d);
          return this.set('weeksleft', w);
        } else {
          this.set('timeleft', this.formatTime(h) + ":" + this.formatTime(m) + ":" + this.formatTime(s));
          return this.set('daysleft', d);
        }
      }
    };
...
(function() {
  ...
  Dashing.LastYear = (function(_super) {
    __extends(LastYear, _super);

    function LastYear() {
      return LastYear.__super__.constructor.apply(this, arguments);
    }

    LastYear.accessor('current', Dashing.AnimatedValue);

    LastYear.accessor('difference', function() {
    ...
    LastYear.accessor('arrow', function() {
      if (this.get('last')) {
    ...
    LastYear.prototype.onData = function(data) {
      if (data.status) {
...
(function() {
  ...
  Dashing.Weekly = (function(_super) {
    __extends(Weekly, _super);

    function Weekly() {
      return Weekly.__super__.constructor.apply(this, arguments);
    }

    Weekly.accessor('current', Dashing.AnimatedValue);

    Weekly.accessor('difference', function() {
      var current, diff, last;
      if (this.get('last')) {
        last = parseInt(this.get('last'));
        current = parseInt(this.get('current'));
    ...
    Weekly.accessor('arrow', function() {
      if (this.get('last')) {
        if (parseInt(this.get('current')) > parseInt(this.get('last'))) {
          return 'icon-arrow-up';
        } else {
    ...
    Weekly.prototype.onData = function(data) {
      if (data.status) {
        $(this.get('node')).attr('class', function(i, c) {
          return c.replace(/\bstatus-\S+/g, '');
        });
